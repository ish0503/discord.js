...
const { table } = require("node:console");
  ...
  module.exports = {
    ...
    async execute(interaction) {
        ...
        const monster = { name: save2[0].name, hp: save2[0].value * 20, reward: Math.round(gambling_find3.money / 10000), XPreward:Math.round(level_find3.level / 1000) };
        const user = { name: save[0].name, hp: save[0].value * 20, reward: Math.round(gambling_find.money / 10000), XPreward:Math.round(level_find.level / 1000) };
        if (save.length <= 0){
